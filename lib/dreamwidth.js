...
class Session {
	...
	return this._get().then( data => {
	    if (!this.valid(data)) throw new DreamwidthError('sid has expired')
	    return data.sid
	}).catch( err => {
	    return this.refresh(err.message)
	})
    }

    valid(data) {
	return (Date.now() - data.timestamp) < this.valid_for
    }
    ...
    _save(_data) { throw new DreamwidthError('override me') }

    _get() { throw new DreamwidthError('override me') }
